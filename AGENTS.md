# AGENTS.md

## Conventions
- Use descriptive variable names
- Keep functions focused and small
- Write code that is self-documenting; add comments only when logic is non-obvious
- Prefer standard library and existing codebase patterns over new dependencies

## Setup
- Dependencies listed in `requirements.txt`
- Use `pip install -r requirements.txt` to set up
- Dataset files already included in the repository

## Project Context
- **Course:** CDS6214 Data Science Fundamentals (Semester 2610)
- **Domain:** Environment, Climate & Sustainability
- **Dataset:** Personal Carbon Footprint (1400 rows, 12 cols)
- **Main deliverable:** `TT3L_G01_Notebook.ipynb`

---

*The work is written by AI*
